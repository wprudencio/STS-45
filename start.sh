...
if ! curl -s --max-time 2 http://127.0.0.1:8080/v1/models >/dev/null 2>&1; then
    ...
    if [[ -n "$LLAMA_BIN" ]]; then
        ...
        if [[ -f "$LLM_MODEL_PATH" ]]; then
            ...
            LLAMA_ARGS="-m \"$LLM_MODEL_PATH\" --host 127.0.0.1 --port 8080 --no-kv-offload -c 16384 -b 512 --cache-type-k q4_0 --cache-type-v q4_0 --reasoning off"
        ...
        echo "   Or start manually: llama-server -m models/gemma-4-E2B-UD-Q2.gguf --host 127.0.0.1 --port 8080 --reasoning off"
    fi
else
